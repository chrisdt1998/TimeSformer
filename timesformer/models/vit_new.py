...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W, register_hook=False):
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            res_temporal = self.drop_path(self.temporal_attn(self.temporal_norm1(xt), register_hook=register_hook))
            ...
            res_spatial = self.drop_path(self.attn(self.norm1(xs), register_hook=register_hook))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, register_hook=False):
        ...
        for blk in self.blocks:
            x = blk(x, B, T, W, register_hook=register_hook)
...
class TimeSformer(nn.Module):
    ...
    def forward(self, x, register_hook=False):
        x = self.model(x, register_hook=register_hook)
        return x
